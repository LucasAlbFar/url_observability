...
- **Drop rules** are layer one and the surgical one: a series whose `handler` or `route` value carries a raw path segment — a bare number, a UUID — is discarded, and the rest of the target is still stored. They select on the **shape of the value**, never on a job name; a rule naming a service would put back in this file the list discovery took out of it. One rule per path-carrying label, because there is no wildcard over label values: `source_labels` names labels explicitly, and `labeldrop`/`labelkeep` match label *names*. Three today — `handler`, `route` and the `http_route` derived from the spans — and only the third outlives the retirement of the other two conventions.
...
**The tracing pillar.** The three services push OTLP over http/protobuf to the Collector on 4318; the Collector writes to Tempo over gRPC on 4317; Grafana reads Tempo through a second datasource, uid `tempo`. The Collector also derives the request metrics from those same spans and publishes them on 8888, where Prometheus pulls them; each service's `/metrics` still carries its process and runtime series, and retiring the three HTTP instrumentations is the rest of the rework. A trace's surface is **Explore**, not a panel: the Collector and Tempo reach *Targets up* and the *Cardinality* row on their own, and no panel was edited to add them. Four rules:
...
Neither `service-node` nor `service-go` gets a route template from its instrumentation — there is no framework to read one from — so both set `http.route` and the span name themselves, from the same value their metric labels use. Do not remove those lines: without them the span is named `GET` and the trace stops saying which route it was.
