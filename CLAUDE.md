...
**The tracing pillar.** The three services push OTLP over http/protobuf to the Collector on 4318; the Collector writes to Tempo over gRPC on 4317; Grafana reads Tempo through a second datasource, uid `tempo`. Metrics are untouched — Prometheus still pulls `/metrics`, and whether the two paths become one belongs to the metrics rework. A trace's surface is **Explore**, not a panel: the Collector and Tempo reach *Targets up* and the *Cardinality* row on their own, and no panel was edited to add them. Four rules:
