...
What survives a `--volumes` teardown: this compose file declares no named volumes, so `--volumes` only removes *anonymous* ones — in practice Grafana's `/var/lib/grafana`, meaning dashboards, users and preferences you created by hand in the Grafana UI are lost. The provisioned Prometheus datasource and the "FastAPI Metrics" dashboard come back on the next `up` because they're bind-mounted from the repo. Bind-mounted repo files are **never** deleted.
